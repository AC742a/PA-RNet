...
from data_provider.data_factory_rb import data_provider
...
class PerturbationSuppressor(nn.Module):
    def __init__(self, dim=768, hidden_dim=512, dropout=0.1, use_residual=True):
        super(PerturbationSuppressor, self).__init__()
    ...
    def forward(self, x, mask):
        """
        x: [batch, seq_len, dim] — e.g., LLM-based text embeddings
        returns: cleaned embeddings of the same shape
        """
        x = x * mask
        perturbation = self.projector(x)  # predict noise component
        if self.use_residual:
            return x - perturbation  # suppress noise
        else:
            return perturbation  # optionally return noise itself
...
class Cross_Attention(nn.Module):
    def __init__(self, d_model, n_heads, d_keys=None, d_llm=None, attention_dropout=0.1, ):
        ...
        self.query_projection = nn.Linear(1, d_keys * n_heads)
        self.key_projection = nn.Linear(d_llm, d_keys * n_heads)
        self.value_projection = nn.Linear(d_llm, d_keys * n_heads)
        self.out_projection = nn.Linear(d_keys * n_heads, 12)
    ...
    def forward(self, target_embedding, source_embedding, value_embedding):
        ...
        source_embedding = self.key_projection(source_embedding).view(B, L, H, -1)
        value_embedding = self.value_projection(value_embedding).view(B, L, H, -1)

        out = self.cross_att(target_embedding, source_embedding, value_embedding)

        out = out.reshape(B, L, -1)

        return self.out_projection(out)

    def cross_att(self, target_embedding, source_embedding, value_embedding):
        B, L, H, E = target_embedding.shape

        scale = 1. / sqrt(E)

        scores = torch.einsum("blhe,bkhe->bhlk", target_embedding, source_embedding)

        A = self.dropout(torch.softmax(scale * scores, dim=-1))
        reprogramming_embedding = torch.einsum("bhlk,blhe->blhe", A, value_embedding)
...
class Exp_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        self.per_sup = PerturbationSuppressor(dim=768, hidden_dim=512, dropout=0.1, use_residual=True).to(self.device)

        self.cross_attn = Cross_Attention(configs.llm_dim, n_heads=8, d_keys=None, d_llm=configs.llm_dim, attention_dropout=0.1,).to(self.device)
        ...
        num_params_per_sup = sum(p.numel() for p in self.per_sup.parameters())
        print(f'Number of parameters in per_sup: {num_params_per_sup}')
        num_params_cross_attn = sum(p.numel() for p in self.cross_attn.parameters())
        print(f'Number of parameters in cross_attn: {num_params_cross_attn}')
        print(f'Total number of parameters: {num_params + num_params_per_sup + num_params_cross_attn}')
        ...
        self.per_sup = self.per_sup.to(self.device)
        self.learning_rate2 = 1e-2
        self.learning_rate3 = 1e-2
    ...
    def _select_optimizer_per_sup(self):
        model_optim = optim.Adam(self.per_sup.parameters(), lr=self.learning_rate2)
        return model_optim

    def _select_optimizer_cross_attn(self):
        model_optim = optim.Adam(self.cross_attn.parameters(), lr=self.learning_rate3)
        return model_optim

    def _select_optimizer_weight(self):
        model_optim = optim.Adam([{'params': self.weight1.parameters()}, {'params': self.weight2.parameters()}], lr=self.args.learning_rate_weight)
        return model_optim
    ...
    def vali(self, vali_data, vali_loader, criterion, all_metric=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, index) in enumerate(vali_loader):
                ...
                batch_text_embeddings, expanded_mask = vali_data.get_text_embeddings(index)
                text_embeddings_rb = self.per_sup(batch_text_embeddings, expanded_mask)
                if self.args.pool_type == "avg":
                    # Mask the embeddings by setting padded tokens to 0
                    masked_emb = text_embeddings_rb * expanded_mask
                    valid_counts = expanded_mask.sum(dim=2, keepdim=True).clamp(min=1)
                    pooled_emb = masked_emb.sum(dim=2) / valid_counts.squeeze(2)
                    text_embeddings_all = pooled_emb.clone()
                elif self.args.pool_type == "max":
                    # Use a large negative number to mask padded positions
                    masked_emb = text_embeddings_rb.masked_fill(expanded_mask == 0, float('-inf'))
                    pooled_emb, _ = masked_emb.max(dim=2)
                    # Handle case where all elements are masked (i.e., -inf), replace with 0
                    pooled_emb[pooled_emb == float('-inf')] = 0
                    text_embeddings_all = pooled_emb.clone()
                elif self.args.pool_type == "min":
                    # Use a large positive number to mask padded positions
                    masked_emb = text_embeddings_rb.masked_fill(expanded_mask == 0, float('inf'))
                    ...
                    text_embeddings_all = pooled_emb.clone()
                # TODO
                att_batch_x = self.cross_attn(batch_x, text_embeddings_all, text_embeddings_all)
                batch_x = torch.cat((batch_x, att_batch_x), dim=-1)
                # decoder input
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float()
                dec_inp = torch.cat([batch_y[:, :self.args.label_len, :], dec_inp], dim=1).float().to(self.device)
                text_dec_inp = torch.zeros((self.args.batch_size, self.args.pred_len, self.text_embedding_dim)).to(
                    self.device)
                text_dec_inp = torch.cat([att_batch_x[:, :self.args.label_len, :], text_dec_inp], dim=1).float().to(
                    self.device)
    ...
    def train(self, setting):
        ...
        model_optim_cross_attn = self._select_optimizer_cross_attn()
        model_optim_per_sup = self._select_optimizer_per_sup()
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, index) in enumerate(train_loader):
                ...
                model_optim_cross_attn.zero_grad()
                model_optim_per_sup.zero_grad()
                batch_x = batch_x.float().to(self.device)
                ...
                batch_text_embeddings, expanded_mask = train_data.get_text_embeddings(index)
                text_embeddings_rb = self.per_sup(batch_text_embeddings, expanded_mask)
                ...
                if self.args.use_amp:
                    ...
                    loss.backward()
                    model_optim.step()
                    model_optim_cross_attn.step()
                    model_optim_per_sup.step()
                    del batch_x, batch_y, batch_x_mark, batch_y_mark, text_embeddings_all, outputs, loss
            ...
            print(
                "Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} Vali Loss: {3:.7f} Test Loss (MSE): {4:.7f} Test MAE: {5:.7f}".format(
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, index) in enumerate(test_loader):
                ...
                if self.Doc2Vec == False:
                    ...
                    input_ids = tokenized_output['input_ids'].view(self.args.batch_size, self.args.seq_len,
                                                                   language_max_len).to(self.device)
                    attn_mask = tokenized_output['attention_mask'].view(self.args.batch_size, self.args.seq_len,
                                                                        language_max_len).to(self.device)
                ...
                if self.Doc2Vec == False:
                    ...
                    text_embeddings_rb = self.per_sup(prompt_emb, expanded_mask)
